...
extern "C"
{
  ...
  #define PIXY_BLOCKTYPE_COLOR_CODE   1
	...
	__declspec(dllexport)  struct Block
  {
    void print(char *buf)
    {
      int i, j;
  ...
  */
  ...
  /**
  @brief      Take a snapshot with pixy.
  @param[in]  mode  mode 0 = 1280x800 25 fps
  @param[in]  X  image capture x offset.
  @param[in]  Y  image capture y offset.
  @param[in]  Width  image capture width
  @param[in]  Height  image capture height
  @param[in]  current_frame pointer to frame buffer memory. (Note 72,000 bytes max on current harware)
  @return     -1    Error

  */
  int  __declspec(dllexport) pixy_cam_getFrame(int mode, int X, int Y, int Width, int Height, unsigned char* *current_frame, uint32_t *size);
  ...
  */
  ...
  /**
    ...
    @return     Negative  Error
  */
  int  __declspec(dllexport) pixy_cam_set_auto_white_balance(uint8_t value);
